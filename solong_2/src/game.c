...
static void	render(t_game *g)
{
	...
	if (info1)
		mlx_string_put(g->mlx, g->win, 10, 20, 0x00FF00, info1); //verde

	coinsleft = ft_itoa(g->remaining_c);	
	info2 = ft_strjoin("Levels Left: ", coinsleft);	
		mlx_string_put(g->mlx, g->win, 10, 40, 0xFFFF00, info2);	//amarillo
	gamename = "My 42 journey";
		mlx_string_put(g->mlx, g->win, 100, 20, 0xFFFFFF, gamename); //blanco
	free(moves_str);
	free(coinsleft);
	free(info2);
	if (info1)
...
static int	try_move(t_game *g, int dx, int dy)
{
	...
	if (dest == 'E')
	{
		...
		return (0); // si no ha recogido todas, ni se mueve ni borra la E
	}
	g->grid[g->py][g->px] = '0';
	g->grid[ny][nx] = 'P';
	g->px = nx; g->py = ny;
